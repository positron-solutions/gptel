...
This package provide common functionality for adapting Emacs to LLM backends, making it easier to build frontends.
** Contributing
PRs are accepted.  Be sure to allow edits when setting up PRs.  You may file issues before working on features to validate the scope / nature of changes or just get help planning.

Target the *experimental branch* unless your changes are fixes for stable releases or release candidates.  The experimental branch will be periodically split into a release candidate branch and stabilized before merge to master.  The experimental branch may have its history re-written.  *Fetch before merging.
** Why a Fork?
...
- Re-focus on pseudo-programming in associated buffers and de-emphasis on tinkering with ephemeral output
...
Topic start and branching context were removed.  The ideas need rework and were in the way.  Persisted content will mostly focus on context & directives as pseudo-programs and structure as steps in pseudo-programs.  Output that is not otherwise persisted in source code or user-created documents is not by default considered useful and will generally be de-emphasized.
...
Before requesting a backend, check if it implements some other backend's API and is compatiable without further changes.
...
- Introspection and logging tools for debugging and dry-runs
...
** Acknowledgments
- [[https://github.com/karthink/gptel][Karthink]] for the original GPTel
